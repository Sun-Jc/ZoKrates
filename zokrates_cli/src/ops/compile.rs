...
pub fn exec(sub_matches: &ArgMatches) -> Result<(), String> {
    ...
    match curve {
        ...
        CurveParameter::Bls12_377 => {
            cli_compile::<Bls12_377Field>(sub_matches)
        }
        CurveParameter::Bls12_381 => {
            cli_compile::<Bls12_381Field>(sub_matches)
        }
        ...
        CurveParameter::Bn254 => cli_compile::<zokrates_field::GrumpkinField>(sub_matches),
    }
}
...
fn cli_compile<T: Field>(sub_matches: &ArgMatches) -> Result<(), String> {
    ...
    let config =
        CompileConfig::default().debug(sub_matches.is_present("debug"));
    ...
    let artifacts =
        compile::<T, _>(source, path.clone(), Some(&resolver), config, &arena)
            .map_err(|e| {
                format!(
                    "Compilation failed:\n\n{}",
    ...
    let r1cs_output_file = File::create(r1cs_output_path).map_err(|why| {
        format!("Could not create {}: {}", r1cs_output_path.display(), why)
    })?;
    ...
    match program_flattened.serialize(&mut bin_writer) {
        Ok(constraint_count) => {
            ...
            let abi_spec_file =
                File::create(abi_spec_path).map_err(|why| {
                    format!(
                        "Could not create {}: {}",
                        abi_spec_path.display(),
                        why
                    )
                })?;
