...
use zokrates_proof_systems::{
    MpcBackend, MpcScheme, TaggedVerificationKey, G16,
};
...
pub fn exec(sub_matches: &ArgMatches) -> Result<(), String> {
    match sub_matches.value_of("curve").unwrap() {
        ...
        BLS12_381 => {
            cli_mpc_export::<Bls12_381Field, G16, Bellman>(sub_matches)
        }
        _ => unreachable!(),
    }
}

pub fn cli_mpc_export<
...
>(
    ...
    let keypair = B::export_keypair(&mut reader)
        .map_err(|e| format!("Could not export keypair: {}", e))?;
