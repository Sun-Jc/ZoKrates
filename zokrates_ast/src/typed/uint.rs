...
impl<'ast, T> Add for UExpression<'ast, T> {
    ...
    fn add(self, other: Self) -> UExpression<'ast, T> {
        ...
        match (self.as_inner(), other.as_inner()) {
            ...
            _ => UExpressionInner::Add(BinaryExpression::new(self, other))
                .annotate(bitwidth),
        }
    }
}
...
impl<'ast, T: Field> UExpression<'ast, T> {
    ...
    pub fn floor_sub(self, other: Self) -> UExpression<'ast, T> {
        let bitwidth = self.bitwidth;
        assert_eq!(bitwidth, other.bitwidth);
        UExpressionInner::FloorSub(BinaryExpression::new(self, other))
            .annotate(bitwidth)
    }
}
...
impl<'ast, T> From<u16> for UExpression<'ast, T> {
    fn from(u: u16) -> Self {
        UExpressionInner::Value(ValueExpression::new(u as u128))
            .annotate(UBitwidth::B16)
    }
}
